...
public class UniDebugExample : MonoBehaviour
{
    ...
    private void TestTagFiltering()
    {
        ...
        DebugLogger.Log("基本タグで出力 - タグは'UniDebugExample'");
        DebugLogger.LogWarning("Warningレベル - タグ'UniDebugExample'");
        DebugLogger.LogError("Errorレベル - タグ'UniDebugExample'");

        // カスタムタグテスト - 2つの方法
        DebugLogger.Log("[Test] タグtest（方法1: メッセージに[タグ]を含む）");
        DebugLogger.Log("タグtest（方法2: パラメータで渡す）", "Test");
        DebugLogger.Log("[Test] メッセージ", "Test2");
    }
